...
- **Localization Director**: Translates episodes into regional Indian languages and produces native-sounding audio for each.

Every stage runs on a custom-built "Senior Story Writer" persona — a skill embedded directly into the OpenAI prompt layer that reasons like an experienced showrunner and editor rather than a generic text model, raising the quality of every continuity check and genre remix it produces.

## Key Features

### 1. Continuity Editor (GPT-4o)
- **Senior Story Writer Skill**: A custom persona embedded in every prompt, instructing the model to reason like an experienced showrunner/editor — not a generic assistant — which materially improves the quality of continuity catches and rewrite suggestions.
...
- **Atmosphere Remixing**: Improvise a script into Noir, Horror, Funny, Drama, or Sci-Fi. The director preserves core plot beats and character identities while altering the descriptive vocabulary and pacing, guided by the same Senior Story Writer persona for consistent creative quality.
...
### 5. Localization Director — Multi-Language Translation & Audio (ElevenLabs)
- **Code-Mixed Translation**: Translates finalized episodes into Kannada, Tamil, and Telugu using natural, spoken regional phrasing — the way people actually talk, code-switching on names and modern terms — rather than stiff textbook translation.
- **Native Multi-Language Audio Generation**: Renders fully narrated audio for each translated version using ElevenLabs' native multi-language voice support, reusing the same parallel-chunked rendering and soundscape-mixing pipeline as the English audio engine.
- **Independent Per-Language Lifecycle**: Each localized version has its own generation and publishing state — publishing one language never affects another, and the original English version is never overwritten.
...
graph TD
    ...
    subgraph AI Pipeline
        Wizard -->|Continuity & Tone| GPT4o["GPT-4o + Senior Story Writer Skill"]
    ...
    subgraph AudioGeneration["Audio Generation"]
    ...
    Localizer -->|Kannada/Tamil/Telugu Translation| AudioGeneration
    Localizer -->|Native Voice Audio| AudioGeneration

    FFmpeg --> Review["Creator Review"]
    Review -->|Explicit Action| Publish["Published Master Track (per language)"]
```
...
| **Translation** | OpenAI GPT-4o | Natural, code-mixed regional-language translation via a dedicated persona. |
| **Audio Synthesis** | ElevenLabs TTS | State-of-the-art prosody and native multi-language support, including Kannada, Tamil, and Telugu. |
...
## Team
Built by **Team Ortex** for the Ortex hackathon. Focusing on AI-powered creator workflows and breaking down barriers to audio production.